...
const CF = (() => {
  ...
  function publicCfg() {
    const fromWindow = (typeof window !== 'undefined' && window.CONTENTFUL_CONFIG) ? window.CONTENTFUL_CONFIG : {};
    return {
      spaceId: fromWindow.spaceId || '',
      deliveryToken: fromWindow.deliveryToken || ''
    };
  }

  function cfg() {
    return { ...publicCfg(), ...localCfg() };
  }

  function saveCfg(obj) {
    const current = localCfg();
    localStorage.setItem(KEY, JSON.stringify({ ...current, ...obj }));
  }
    ...
    const types = [
      ...
      },
      {
        ...
        fields: [
          ...
          { id: 'sizeBio',             name: 'Bio size',                type: 'Integer' },
          { id: 'sizePubs',            name: 'Publications size',       type: 'Integer' },
          { id: 'sizePhotoIntro',      name: 'Photography intro size',  type: 'Integer' },
          { id: 'sizeLabels',          name: 'Section labels size',     type: 'Integer' },
      ...
      },
      {
        ...
        fields: [
          { id: 'key',        name: 'Key',         type: 'Symbol',  required: true },
          { id: 'heroBanner', name: 'Hero Banner URL', type: 'Symbol' }
        ],
        displayField: 'key'
      },
      {
